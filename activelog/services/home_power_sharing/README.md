...
```bash
# Using the CLI
python dev-tools/cli/activelog_cli.py services start home_power_sharing

# Direct execution
cd services/home_power_sharing
python -m uvicorn main:app --host 0.0.0.0 --port 8440 --reload
```
...
```bash
# Run service tests
python -m pytest tests/services/home_power_sharing/

# Run with coverage
python -m pytest tests/services/home_power_sharing/ --cov=services/home_power_sharing
```
